...
#define	MAX_FEATURES_MONITORED	20
int	num_features_monitored = 0;
int	features_monitored[MAX_FEATURES_MONITORED];
int	fm_status[MAX_FEATURES_MONITORED];
time_t	fm_time[MAX_FEATURES_MONITORED];
...
char *argv[];
{
...
#ifdef DEBUG
	{
		...
		DebugFlags[0] = "U  UNIX communications";
		DebugFlags[1] = "D  PHONE";
		DebugFlags[2] = "H  HELP";
		DebugFlags[3] = "I  INFO";
		DebugFlags[4] = "L  LAMP";
		DebugFlags[5] = "T  TIMER";
		DebugFlags[6] = "M  MOUSE";
		DebugFlags[7] = "S  SPEEDLIST";
		DebugFlags[8] = "R  Raw phone line";
	}
#endif /* DEBUG */
	...
	for (opt=*++argv; opt && *opt; opt=*++argv)
	{
		...
		} else if (opt[0]=='-' && opt[1]=='F')
		{
			if (opt[2])
				...
				break;
			if (num_features_monitored  == MAX_FEATURES_MONITORED) {
				/* should really print an error msg */
				break;
			}
			/* The feature number should be in HEX */
			features_monitored[num_features_monitored] = (int) strtol(f, (char **)NULL, 16);
			fm_status[num_features_monitored] = -1;
			num_features_monitored++;
#ifdef DEBUG
	...
	*PhoneBufPtr = '\0';
	...
	Options[2].text = "open window on incoming call";
	Options[2].flag = &MakeTop;
	Options[3].text = "open window on outgoing call";
	Options[3].flag = &OpenIt;
	Options[4].text = "leave window open after hangup";
	...
	{
		LineApps[i].status = 0;
		LineApps[i].line = -1;			/* No lineapp yet */
		LineApps[i].direction = IS_NONE;
	}

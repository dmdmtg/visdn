...
extern	FILE	*Pifp,		/* Phone line input */
		...
		*Xofp,		/* Xtentol output */
		*Pipefp;	/* command pipe input */
extern int  DispMode;
#endif	/* DMD630 */

extern char	*Progname;
extern void HayesMsgProc(), HayesSend();
...
int	speaker_on = 0, adjunct_on = 0, handset_up = 0;
...
#ifdef	DMD630		/* To make sharing code easier */
...
{
	if (*s == '=') {
		/* Is a pq-style phone number, so it won't
		 * have any letters.
		 * SnarfDial requires numbers >= 7 digits long
		 * and will return 0 if "s" isn't what it
		 * thinks is a valid phone number.
		*/
		(void) SnarfDial( ++s, MAINITEM 1 );
	} else {
		/* A phone number which may include a-z,
		 * e.g., "1800CALLATT".  If so, convert
		 * the letters to numbers.
		 * Also, remove the characters " -()" that often
		 * appear in phone numbers.
		*/

		char	number[200], *n;
		char *t, *strchr(), az_to_09();

		(void) strcpy(number, "CA");
		if( t = strchr( s, '\n' ) )
			*t = 0;
		for (t=s, n=&number[2]; *t; t++) {
			switch (*t) {
			case ' ': case '-':
			case '(': case ')':
				continue;	/* ignore it */
		...
		*n = '\0';
		/* Try SnarfDial() without the leading "CA" since it adds one.
		 * If it fails there's no area code or full exchange,
		 * which means we can just dial the number as is.
		*/
		if (SnarfDial( &number[2], MAINITEM 1) == 0) {
...
PhoneProc()
{
	...
	Trynumber = 0;
	Connectseconds = 20;
	Retryseconds = TICKS_PER_SECOND*2;
	...
	InitDisplay(Type);
	SendHost(PROC_PHONE, INITTERM, (char *)0, EOM);
...
void HandlePhoneEvent()
{
	...
	if (PhoneEvent & EVENT_RCVBUF)
	{
		...
		case CAESCAPE:
			Escape = String(s);
			break;
		case ERRORMSG:		/* display an error message from UNIX */
			LCD.Setmsg(LCD_MSG_FULLDISP, M_NORMAL, s, (struct LineApp *)NULL);
			break;

		case UNIXLEAVING:	/* The UNIX side is leaving so don't try */
					/* to communicate with it anymore */
			if (*s != '\0')
				LCD.Setmsg(LCD_MSG_FULLDISP, M_NORMAL, s, (struct LineApp *)NULL);
/*XXXXXXXXXXXXXX*/
			break;

		case DIALNUM730:	/* This actually comes from the pipe */
			DPRINTF("\nGot DIALNUM730 '%s'\n", s);
			(void) do_dialnum(s);
			break;
		case RMTACT730:		/* This actually comes from the pipe */
			DPRINTF("\nGot RMTACT730 '%s'\n", s);
			rmtact( s );
			break;

		default:
			/* (at least for now), ignore unknown commands */
			break;
		}
...
#ifdef	NOT_USED	/* no one uses this routine */
/*
...
#define	RCVMSG(cp, c, fd) \
	for (cp=s; read(fd, &c, 1)==1 && c != '\n'; cp++) { \
			*cp = c; \
		} \
		*cp = '\0'
/* Process the input from the command-line pipe. */
void
process_pipe_input()
{
	char	c, s[200], *cp;
	...
	if (read(fd, &c, 1) == 1) {
		...
		case DIALNUM:		/* octal 0356 */
			RCVMSG(cp, c, fd);
			DPRINTF("Got DIALNUM '%s'\n", s);
			(void) do_dialnum(s);
			break;
		case RMTACT:		/* octal 0357 */
			RCVMSG(cp, c, fd);
			DPRINTF("Got RMTACT '%s'\n", s);
			rmtact( s );
			break;
		default:
			(void) fprintf(stderr, "Unknown command from the command pipe: 0x%x (%c)\n", (int)c, c);
		break;
		}
	}
}

/* Wait for input from the phone or the pipe.
 * Return 1 if there's pipe input, else 0.
*/
static int
is_pipe_input()
{
#ifdef	NO_POLL
	return(0);	/* No pipe, hence no pipe input. */
#else

	while (poll(fds, num_fds, INFTIM) != -1) {
		int	revents;
		char	*where;

		if (fds[1].revents & POLLIN) {		/* pipe input */
			return(1);
		}
		if (fds[0].revents & POLLIN) {		/* phone input */
			return(0);
		}

		/* If we get here, poll() returned success, but there
		 * wasn't anything to read.  There was probably some unexpected
		 * message or EOF.
		 * If we ever get errors on BOTH fds, then the user will only
		 * see a message for the 2nd one, but I doubt we'll ever get
		 * errors on both at the same time.
		*/
		revents = 0;
		if (fds[0].revents != 0) {
			where = "phone";
		...
		if (revents) {
			...
			*/
			...
			sleep(30);	/* so maybe they'll see the phone LCD */
	...
	perror("poll");
	/* Kill the X process, which will eventually kill us. */
	(void) fprintf(Xofp, "xtent.XtExit: 0\n");
	return(0);
...
PhoneProc()
{
	...
	while (1)
	{
		...
		/* Check for phone and/or pipe input.
		 * If there's pipe input, handle it and wait again.
		*/
		if (is_pipe_input()) {
			process_pipe_input();
			continue;
		...
		if (fgets(PhoneInBuf, sizeof(PhoneInBuf), Pifp) != NULL)
		{
			...
			*/
			if (errno == 0) {
				...
				if (got_line == 1) {
					(void) fprintf(stderr, "\n");
					(void) fprintf(stderr, "%s: Read of phone line failed!\n", Progname);
...
char *str;
{
...
#ifndef DMD630
	/* The X version has a trailing \n, so zap it */
	str[strlen(str)-1] = '\0';
#endif
	/* optionally save the line in the debugging file */
	RPRINTF("<< %s\n", str);
	...
	case TYPE_FEATURE:	/* Feature lamp Selection */
		...
		/* The following line of code (Lamp Control function call) was
		 * previously commented out.  There was no comment explaining
		 * why it was commented out.  The code was put back in to fix
		 * a problem with lamps not working on the VISDN window for
		 * buttons that do not physically have lamps on the ISDN 7506.
		 * ProPhone generates ONLY a "FEATURE:" message if the set
		 * does not actually have a lamp for the button.  ProPhone
		 * generates a "FEATURE:" message and an "AT&T LAMP:" message
		 * if the set has a lamp for the button.  Putting this code
		 * back in allows the lamp status for the "Rtn Last Out"
		 * button (#17) to be properly shown in the VISDN window when
		 * a Camp-On is active (since only a "FEATURE:" message is
		 * generated and it will be handled here).  In order for lamps
		 * to work properly, the button must be defined correctly in
		 * the .visdnrc file (you need button=, name=, value=*B<num>,
		 * box, and lamp=<button-number-in-hex>).
		 *
		 * Daniel Beitz, 11/17/95
		 */
 ...
 * Don't do it yet because it can break the case where you pick up the phone
 * and put it down with out making a call.
*/
#ifdef	DMD630
		if (MakeTop)
		{
	...
	case TYPE_SIGNAL:	/* line signal received */
		...
		*/
		...
		case SIG_ALERT3:	/* ringing pattern 3 */
			...
			/*FALLTHROUGH*/
		case SIG_ALERT4:	/* ringing pattern 4 */
		case SIG_ALERT5:	/* ringing pattern 5 */
		case SIG_ALERT6:	/* ringing pattern 6 */
		case SIG_ALERT7:	/* ringing pattern 7 */
			/* This PRINTF assumes the SIG_ALERT? #defines go
			 * up in consecutive value by 1.
			*/
			DPRINTF(" ringing (pattern %d)", mdata[2] - SIG_ALERT0);
			break;
		case SIG_NOALERT:
		...
		/* These are just here for debugging -- don't do anything */
		case SIG_ANSWER:
			DPRINTF(" answer tone on");
			break;
		case SIG_CONFIRM:
			DPRINTF(" confirm tone on");
			break;
		case SIG_BUSY:
			DPRINTF(" busy tone on");
			break;
		case SIG_STUTTER:
			DPRINTF(" stutter dial tone on");
			break;
		case SIG_ERROR:
			DPRINTF(" error dial tone on");
			break;

			/* I'm not sure what they all do, so I didn't add
			 * all of them.
			*/
		default:
			DPRINTF(" (type 0x%x)", mdata[2]);
			break;
		}
		DPRINTF("\n");
		return;
...
#ifdef	USE730WINDOWS
		/* If we shouldn't leave the window on top after the last
		 * call is over, see if this is the last active call.
		*/
		if ( ! LeaveTop)
		{
			register int i;
			...
			if (i == MAXCALLAPPS) {	/* no active connections */
/* XXXX FIX: really should only do this if the window was tiled when we
 *	got or made the call.
*/
				tile730wins( P );
			}
		}
...
XXXXXXXX for now, don't do this.  I'm not 100% sure the following comment
	...
	case TYPE_SWITCHHOOK:	/* Switchhook Transitions */
		...
		case SH_SPEAKER_ON:
			DPRINTF("(SPEAKER ON)\n");
#ifdef	USE730WINDOWS
		/* XXX	Why isn't thise untile730wins code also with the REMOTE_ON
		 *	and ADJUNCT_ON ???
		*/
			if ( OpenIt )
				untile730wins( P );
...
int event;
{
	...
	*/
	...
	/* In case the log routine doesn't log what we sent it,
	 * optionally write the record to the debug file.
	 * We can then determine why the logger didn't work.
	*/
	DPRINTF(">>> SENT TO LOGGER: %s\n", sendbuf);

#ifdef	DMD630
	SendHost(PROC_PHONE, CALLINFO, sendbuf, EOM);     /* send to host to log */
#else
	LogInfo(mailaddr, sendbuf);
#endif	/* DMD630 */
...
#ifdef	DMD630
	if( event == 0 && ( lp->status&CONNECTED) == 0 && *lp->callingdn)
	{
		int n;
		char buf[ 120 ];
		char snarfbuf[48];
		...
		/* Add this incoming "missed call" to the speedlist. */

		area[0] = exch[0] = ext[0] = '\0';
		(void) strcpy( snarfbuf, lp->callingdn );
		MPRINTF("Add Missed '%s'\n", snarfbuf);

/* XXXX	Don't ask me why, but regex sometimes returns NULL on SUCCESS,
 * 	but it's supposed to return it on FAILURE.
 *	And sometimes it works correctly!!!!!
 *	It seems like often when I add/change some code,
 *	then regex's behavior changes.  Almost like it depends where it is
 *	on the stack, or something like that.
*/
		/* If regex() works, "re" points to the next unmatched character
		 * and "__loc1" points to where the match began.
		*/
		re = regex(snarfstr, snarfbuf, area, exch, ext);
/*XXXYYY
MPRINTF("__loc1=0x%lx (%s), re=0x%lx (%s)\n",
(long)__loc1, __loc1 ? __loc1 : "null",
(long)re,  re ? re : "null");
XXXYYY*/
		if (re == (char *)NULL) {
			MPRINTF("regex(%s) failed\n", snarfbuf);
		} else {
			MPRINTF("regex(%s) returned: %s %s %s\n", snarfbuf, area, exch, ext);
		...
		if (feature == features_monitored[i]) {
			...
			*/
			if (status != fm_status[i]) {
				...
				now = realtime();
#ifdef	DMD630
				now /= TICKS_PER_SECOND;	/* convert to seconds */
#endif	/* DMD630 */
				...
				DPRINTF(" (was %d for %s))\n", fm_status[i], t);
				(void) sprintf(sendbuf, "FEATURE\t%x\t%s\t%s", feature, s, t);
#ifdef	DMD630
				SendHost(PROC_PHONE, FEATURE_STATUS, sendbuf, EOM);
#else
				FeatureStatus(sendbuf);
#endif	/* DMD630 */
				fm_status[i] = status;
				fm_time[i] = now;
			} else
...
void HayesXmit()
{
	...
	if (oqp->flags & OQ_WAIT_FOR_TONE)
	{
		...
		/* We have dialtone, even though we may not have timed out yet. */
		DPRINTF("OQ dialtone timeout status %x, %lx-%lx=%d, tmout=%d\n",
			lp->status,
			realtime(), PhoneOutQueue.start_time,
			(int)(realtime()-PhoneOutQueue.start_time), oqp->timeout);
	...
	if (oqp->flags & OQ_WAIT_FOR_CONN)
	{
			...
			(realtime()-PhoneOutQueue.start_time) < oqp->timeout)))
		{
			...
			if (lp->status & BUSY) {
				...
				DPRINTF("OQ conn timeout status %x, %lx-%lx=%d >= %d\n",
					lp->status,
					realtime(), PhoneOutQueue.start_time,
					(int)(realtime()-PhoneOutQueue.start_time), oqp->timeout);
			}

			HayesSend(Hangupstring, strlen(Hangupstring), 0);
			...
			oqp->timeout = Retryseconds*TICKS_PER_SECOND;
			return;
		}
	}
	...
	if (oqp->flags & OQ_WAIT_FOR_RETRY)
	{
		...
		DPRINTF("OQ retry timeout status %x, %lx-%lx=%d >= %d\n",
			lp->status,
			realtime(), PhoneOutQueue.start_time,
			(int)(realtime()-PhoneOutQueue.start_time), oqp->timeout);
		HayesSend(Redialstring, strlen(Redialstring), 0);
		PhoneOutQueue.start_time = realtime();
		oqp->timeout = Connectseconds*TICKS_PER_SECOND;
		oqp->flags &= ~OQ_WAIT_FOR_RETRY;
		oqp->flags |= OQ_WAIT_FOR_CONN;
		return;
	}
	...
	if (oqp->flags & OQ_NEED_CA)
	{
		...
		oqp->flags |= OQ_WAIT_FOR_TONE;
		oqp->timeout = 2*TICKS_PER_SECOND;
		DPRINTF("Need CA for '%s' tmout=%d rt=%lx\n",
			oqp->buf ? oqp->buf : "(null)",
			oqp->timeout, realtime());
	...
	DPRINTF("HayesXmit flags=0x%x buf='%s'\n", oqp->flags,
		oqp->buf ? (*oqp->buf ? oqp->buf : "(empty)") : "(null)");
	...
	while (*cp)
	{
		...
		case '+':	/* send first part and wait for dialtone */
			...
			/* wait for 2ndary dialtone */
			PhoneOutQueue.start_time = realtime();
			oqp->timeout = 2*TICKS_PER_SECOND;	/* 2 sec timeout */
		...
		case '~':	/* to be compatible with FP3 version */
			...
			PhoneOutQueue.start_time = realtime();
			oqp->timeout = TICKS_PER_SECOND/2;	/* 1/2 sec timeout */
			oqp->flags |= OQ_PAUSE;
...
int f;
{
	...
	if (n == 0) {
		DPRINTF("HayesSend(): asked to send 0 chars; returning\n");
		return;
	}
	...
	*cp++ = '\0';
	...
	/* Don't add quotes around %s since obuf has a \r in it,
	 * and it messes up the debug output.
	*/
#ifdef	DMD630
...
#else	/* is not DMD630 */
	DPRINTF("HayesSend() sending: %s\n", obuf);
	(void) fputs(obuf, Pofp);
	(void) fflush(Pofp);
#endif	/* DMD630 */
}

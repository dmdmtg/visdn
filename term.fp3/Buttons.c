...
register char *s;
{
	...
	*cp = '\0';
	...
	fbp->value2[0]='\0'; /* init for fp3, assume no second value */
	for (cp=fbp->value; cp < &fbp->value[sizeof(fbp->value)-1] && *s && *s != ';'; ){
		if(*s != '|'){
			*cp++ = *s++;
		}else{
			*cp = '\0';
			s++; /* advance to second value */
			/* for value2 added for fp3, see Buttons.h  */
			for (cp=fbp->value2; cp < &fbp->value2[sizeof(fbp->value2)-1] && *s && *s != ';'; )
				*cp++ = *s++;
			*cp = '\0';
		}
	}
...
FeatBut *fp;
{
	...
	if (fp->arg==(char *)0 || *fp->arg=='\0')
		...
		/* need two messages sent in response to but press, eg, keypad */
		DPRINTF("ButtonPress fp.arg is %s \n",fp->arg);
		DPRINTF("ButtonPress fp.arg2 is %s \n",fp->arg2);
		OutEnQueue(&PhoneOutQueue, fp->arg);
		return OutEnQueue(&PhoneOutQueue, fp->arg2);
	}else{
		DPRINTF("ButtonPress fp.arg is %s \n",fp->arg);

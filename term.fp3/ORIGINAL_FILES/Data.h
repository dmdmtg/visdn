 ...
 *
 * P is global variable in 630/730 OS
 * pointing to process structure (struct Proc) associated with
 * the currently running process 
 * ? appl pointer to user data, any constraints on use?
 */

#define	GLOBREF(x)	(((struct GlobalData *)P->appl)->x)

/* Global data structure referenced through process structure P */

struct GlobalData {
	DisplayMod	GD_LCD; /* struct for phone LCD display */
	LampMod		GD_LAMP; /* struct for LEDs/lamps */
	...
	char *		GD_TuneString;	/* added for fp3 */
	char *		GD_Escape;	/* added for fp3 */
	int		GD_AlertPat;	/* added for fp3 */
};
...
#define TuneString	GLOBREF(GD_TuneString)	/* added for fp3 */
#define Escape		GLOBREF(GD_Escape)	/* added for fp3 */
#define AlertPat	GLOBREF(GD_AlertPat)	/* added for fp3 */

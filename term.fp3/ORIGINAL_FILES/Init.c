...
char *FButs7506[] =
{
	...
	"3,1;c;*B3;bL03;;",
	"4,1;Message;;nLM;;",
	"5,1;Confrnce;&&x3,1,1;s;;", /* updated for fp3 */
	"6,1;Drop;&&x3,2,1;s;;", /* updated for fp3 */
	"1,2;d;*B4;bL04;;",
	"2,2;e;*B5;bL05;;",
	"3,2;f;*B6;bL06;;",
	"4,2;DATA;*B7;bL07;;",
	"5,2;Transfer;&&x3,3,1;s;;", /* updated for fp3 */
	"6,2;Hold;&&x3,4,1;s;;", /* updated for fp3 */
	"1,3;h;*B8;bL08;;",
	"2,3;i;*B9;bL09;;",
	"3,3;j;*B10;bL0a;;",
	"4,3;k;*B11;b;;",
	"5,3;l;*B12;b;;",
	"6,3;m;*B13;b;;",
	"7,3;Redial;&&x4,5,1;b;;", /* updated for fp3 */
	"8,3;Shift;&&x4,2,1;bLT;;", /* updated for fp3 */
	"9,3;Mute;&&x4,1,1;bLU;;", /* updated for fp3 */
	"10,3;Spkr;&&x4,6,1;bLS;;", /* changed fp3, for speaker phone */
	"11,3;v  Vol;&&x4,8,1;<;;", /* updated for fp3 */
	"11,3;   ^;&&x4,7,1;>;;", /* updated for fp3 */
	NULL,
};
...
char *FButs7507[] =
{
	...
	"9,2;L;*B39;bL27;;",
	"10,2;M;*B40;bL28;;",
	"11,2;v  Vol;&&x4,8,1;<;;", /* updated for fp3 */
	"11,2;   ^;&&x4,7,1;>;;", /* updated for fp3 */

	"1,3;Confrnce;&&x3,1,1;s;;", /* updated for fp3 */
	"2,3;Drop;&&x3,2,1;s;;", /* updated for fp3 */
	"10,3;Message;;nLM;;",
	"11,3;Spkr;&&x4,6,1;bLS;;", /* changed fp3, for speaker phone */

	"1,4;Transfer;&&x3,3,1;s;;", /* updated for fp3 */
	"2,4;Hold;&&x3,4,1;s;;", /* updated for fp3 */
	"10,4;Select;&&x4,2,1;bLT;;", /* updated for fp3 */
	"11,4;Mute;&&x4,1,1;bLU;;", /* updated for fp3 */
	...
	"8,5;r;*B18;bL12;;",
	"9,5;s;*B19;bL13;;",
	"10,5;Redial;&&x4,5,1;b;;", /* updated for fp3 */
	"11,5;DATA;;bLX;;",
...
char *argv[];
{
	...
	if ((tmem = alloc(sizeof(struct GlobalData)))==0){
		DPRINTF("NO MEMORY FOR GLOBAL DATA\n");
		exit();
	}
	P->appl = (long)tmem;
	...
	CacheOnExit = MakeTop = SendAUDIXMsg = 0;
#ifdef DEBUG
	{
	...
	for (opt=*++argv; opt && *opt; opt=*++argv)
	{
		if (opt[0]=='-' && opt[1]=='p')
		{
			/* option -px where x is port number */
			/* HostPort is the phone's port?     */
			if (opt[2])
				HostPort = atoi(&opt[2]);
			else if (opt = *++argv)
			{
		...
		{
			/* option -A for sending AUDIX message */
			SendAUDIXMsg = 1;
		} else if (opt[0]=='-' && opt[1]=='C')
		{ 
			/* cache visdn in terminal memory */
			CacheOnExit = 1;
		} else if (opt[0]=='-' && opt[1]=='P')
		{
			/* pop window to top on incoming call */
			MakeTop = 1;
		} else if (opt[0]=='-' && opt[1]=='c')
		{
			/* number of feature button columns */
			if (opt[2])
				ReqCol = atoi(&opt[2]);
			else if (opt = *++argv)
			{
	...
	*PhoneBufPtr = '\0';
	...
	Options[2].text = "pop layer to top on ring";

 ...
 *	the definition of the logical (Fw_Logio) and physical (Pysio) IO block in this file.
*/
...
typedef struct Fw_Logio {
	...
	long pad1[8];		/* padding, in case chanproc or Fw_Logio itself
				** need expansion
				*/
...
#define NLOGIOS		4
#ifdef INIT_C
struct Fw_Logio logio[NLOGIOS];
#else
extern struct Fw_Logio logio[NLOGIOS];
#endif
...
typedef struct Pysio {
	...
	int mnid;		/* minor id */
	struct Fw_Logio *host;	/* shortcut pointer to logical IO block */
...
struct Fw_Logio *Maplog[LOG_COUNT];	/* Mapping array */
#else
extern int Num_devices;
extern int Num_hosts;
extern int Num_printers;
extern struct Fw_Logio *Maplog[LOG_COUNT];
#endif

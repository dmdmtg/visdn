...
static char *MailAddr = NULL;

extern char *HostVersion;
#ifdef __STDC__
extern char *HostDate, *HostTime;
#endif
static char EraseChar, KillChar;
...
int	pipepid, ttypid;
#else
static	struct pollfd	fds[2];		/* read from 730 and pipe */
static	size_t	num_fds;
#endif	/* NO_POLL */
...
char *argv[];
{
	...
	EraseChar = tbuf.c_cc[VERASE];
	KillChar = tbuf.c_cc[VKILL];
...
int	is_main;
{
...
#ifdef	NO_POLL
	fd = 0;		/* parent process reads from here */
#else
	int	timeout;
	size_t	numfds;

	if (is_main) {
		/* This is the main loop, so check both the 730 and pipe for
		 * input, and hang until there is some.
		*/
		timeout = INFTIM;
		numfds = num_fds;
	} else {
		/* We were NOT called from the main loop, and we're really
		 * just trying to keep the 730 input buffer from filling,
		 * so just check the 730 fd (the 1st one).
		*/
		numfds = 1;
		timeout = 0;	/* return immediately if no input chars */
	}
	...
	*/
	...
	if (pollret == -1) {
		perror("poll");
		/* let the 730 know we're leaving */
		SendCmd(UNIXLEAVING, "UNIX process died (poll failed)\n", 0);
		DoExit(2);
	}
	...
	if (fds[0].revents & POLLIN) {
		...
		if (revents) {
			 ...
			 * If we can't read from the 730, we probably
			 * can't write to it either, but try anyway.
			*/
			char	msg[60];
			(void) sprintf(msg, "Unable to read from %s (revents=0x%x)\n", where, revents);
			/* let the 730 know we're leaving */
			SendCmd(UNIXLEAVING, msg, 0);
	...
	if (read(fd, &c, 1)==1)
	{
		...
		case QUITCMD:
			...
			/* NOTREACHED */
		case INITTERM:
			SendCmd(ERASECHAR, (char *)&EraseChar, 1);
			...
			*/
			SendCmd(IGNORECMD, "\n", 0);
			SendCmd(KILLCHAR, (char *)&KillChar, 1);
			SendCmd(IGNORECMD, "\n", 0);
...
/* I'm not sure if this is a good idea or not. */
	...
	return(1);	/* 1 == there was something to do */
}
